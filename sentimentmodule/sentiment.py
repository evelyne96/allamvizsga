    ...
    if os.path.exists(filename):
        with open(filename) as file:
            ...
            for row in reader:
                ...
                if (row[1] != 'Sentiment'):
                    modelList.append((int(row[1]),row[2]))
    return modelList

def readTrainData():
        filename = os.path.join(os.path.abspath('..'), 'static', 'sentimentAnalysis', 'train.csv')
...
def classify():
        ...
        params = {'C': 1.7782794100389228, 'gamma': 0.5336699231206302}    
        ...
        # pickle.dump(svm_cls, open('svm_rbf_classifier.pkl', 'wb'))
        # pickle.dump(nb_cls, open('nb_classifier.pkl', 'wb'))
        # pickle.dump(vectorizer, open('vectorizer.pkl', 'wb'))
        # classifiers = [(svm_cls, 'SVM with rbf'), (nb_cls, 'Multinomial Naive Bayes')]
        # test_classifier_accuracy(classifiers, vectorizer, test)
...
def test_classifier_accuracy(list_of_cls, vectorizer, testData):
    input_data, true_prediction = get_test_data(testData, vectorizer)
    for (c, name) in list_of_cls:
        predictions = get_prediction(c, input_data)
        ac, f1 = get_metrics_for(predictions, true_prediction)
        print(name + ' : accuracy %f, f1 score %f' % (ac, f1))

def get_test_data(testData, vectorizer):
    true_predictions = [s for (s, text) in testData]
    input_data = vectorizer.transform([text for (s, text) in testData])
    return input_data, true_predictions


def get_metrics_for(prediction, true_prediction):
    accuracy = accuracy_score(true_prediction , prediction)
    f1_acc = f1_score(true_prediction, prediction)
    return accuracy, f1_acc

def get_prediction(classifier, input_data):
       p = classifier.predict(input_data)	
       return p

#preproceccing data
def eliminateStopWords(trainData):
        for index, (sentiment,sentimentText) in enumerate(trainData):
            ...
            x = list(trainData[index])
            x[1] = sentimentText
            trainData[index] = tuple(x)
...
def visualize_parameters(train_features, trainData,params):
    ...
    plt.title("gamma=%f, C=%f" % ((params['gamma']), params['C']),
              size='medium')
